...
bool Point::Distance(int i,int j,Point V,int n){
	...
	if(i==Capital&&j>0&&j<Municipality&&distance<MAX_DISTANCE){  //Ca-M
		return true;
	}
	else if(i==Capital&&j>=Municipality&&j<County&&distance<MAX_DISTANCE*0.6){ //Ca-C
		return true;	
	}
	else if(i==Capital&&j>=County&&distance<MAX_DISTANCE*0.45){	//Ca-T
		return true;
	}
	else if(i>Capital&&i<Municipality&&j>Capital&&j<Municipality&&distance<MAX_DISTANCE*0.9){ //M-M
		return true;
	}
	else if(i>Capital&&i<Municipality&&j>=Municipality&&j<County&&distance<MAX_DISTANCE*0.5){ //M-C
		return true;
	}
	else if(i>Capital&&i<Municipality&&j>County&&distance<MAX_DISTANCE*0.4){ //M-T
		return true;
	}
	else if(i>=Municipality&&i<County&&j>=Municipality&&j<County&&distance<MAX_DISTANCE*0.8){ //C-C
		return true;
	}
	else if(i>=Municipality&&i<County&&j>County&&distance<MAX_DISTANCE*0.35){ //C-T
		return true;
	}
	else if(i>County&&j>County&&distance<MAX_DISTANCE*0.7){ //T-T
		return true;
	}else return false;
